...
def make_document(project, uploader, name='test.txt', status='pending'):
...
class DocumentUploadViewTests(TestCase):
    ...
    def test_upload_nonexistent_project_returns_400(self):
        c = auth_client(self.owner)
        f = fake_txt_file('note7.txt')
        import uuid
        resp = c.post(self.url, {'file': f, 'project_id': str(uuid.uuid4())}, format='multipart')
...
class DocumentDetailViewTests(TestCase):
    ...
    @patch('documents.embeddings.get_or_create_collection')
    @patch('documents.embeddings.get_chroma_client')
    def test_owner_can_delete_document(self, mock_client, mock_col):
