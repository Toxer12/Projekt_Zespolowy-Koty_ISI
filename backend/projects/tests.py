...
def make_user(email, password='Pass1234x', name=None, is_active=True):
...
class ProjectInviteCreateViewTests(TestCase):
    ...
    def setUp(self):
        self.owner = make_user('invown@proj.com', name='invown')
        self.project = make_project(self.owner, 'InvProj')
        self.invitee = make_user('invitee@proj.com', name='inviteeuser')
    ...
    def test_viewer_cannot_invite(self):
        viewer = make_user('viewer2@proj.com', name='viewer2user')
        add_member(self.project, viewer, 'viewer')
        c = auth_client(viewer)
        resp = c.post(self.url, {'username': 'inviteeuser', 'role': 'viewer'})
        self.assertEqual(resp.status_code, 403)
    ...
    def test_admin_can_invite_editor(self):
        admin = make_user('admin2@proj.com', name='admin2user')
        add_member(self.project, admin, 'admin')
        invitee2 = make_user('inv2@proj.com', name='inv2user')
        c = auth_client(admin)
        resp = c.post(self.url, {'username': 'inv2user', 'role': 'editor'})
        self.assertEqual(resp.status_code, 201)

    def test_admin_cannot_invite_admin(self):
        admin = make_user('admin3@proj.com', name='admin3user')
        add_member(self.project, admin, 'admin')
        invitee3 = make_user('inv3@proj.com', name='inv3user')
        c = auth_client(admin)
...
class InviteRespondViewTests(TestCase):
    ...
    def setUp(self):
        self.owner = make_user('resown@proj.com', name='resown')
        self.project = make_project(self.owner, 'ResProj')
        self.invitee = make_user('resitee@proj.com', name='resitee')
...
class InviteListViewTests(TestCase):
    ...
    def setUp(self):
        self.owner = make_user('listinvown@proj.com', name='listinvown')
        self.project = make_project(self.owner, 'ListInvProj')
        self.invitee = make_user('listinvitee@proj.com', name='listinvitee')
        ProjectInvite.objects.create(
            project=self.project, invited_by=self.owner,
...
class InviteCancelViewTests(TestCase):
    ...
    def setUp(self):
        self.owner = make_user('canown@proj.com', name='canown')
        self.project = make_project(self.owner, 'CanProj')
        self.invitee = make_user('canitee@proj.com', name='canitee')
        self.invite = ProjectInvite.objects.create(
            project=self.project, invited_by=self.owner,
            invitee=self.invitee, role='editor',
        )
