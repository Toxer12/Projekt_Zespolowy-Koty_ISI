...
def make_user(email='test@example.com', password='Test1234', name='testuser', is_active=True):
...
class UserManagerTests(TestCase):
    ...
    def test_create_user_success(self):
        user = User.objects.create_user(email='a@b.com', password='Pass1234', name='abc')
...
class RegisterViewTests(TestCase):
    ...
    def test_register_success_sends_email(self, mock_send):
        data = {'email': 'new@example.com', 'password': 'Secure1pass', 'name': 'newuser'}
    ...
    def test_register_duplicate_email_fails(self, mock_send):
        ...
        data = {'email': 'dup@example.com', 'password': 'Secure1pass', 'name': 'dup'}
        resp = self.client.post(self.url, data)
        self.assertIn(resp.status_code, [400, 422])

    @patch('users.views.send_mail')
    def test_register_weak_password_fails(self, mock_send):
        data = {'email': 'weak@example.com', 'password': 'weak', 'name': 'weakuser'}
        resp = self.client.post(self.url, data)
    ...
    def test_register_email_failure_doesnt_break_registration(self, mock_send):
        ...
        data = {'email': 'ok@example.com', 'password': 'Secure1pass', 'name': 'okuser'}
        resp = self.client.post(self.url, data)
        self.assertEqual(resp.status_code, 201)
...
class MyViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = make_user(email='my@example.com', password='MyPass1', name='myuser')
